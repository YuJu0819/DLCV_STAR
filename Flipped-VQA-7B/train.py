...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--llama_model_path', default='./pretrained/Sheared-LLaMA-1.3B.pth', type=str, help='path of llama model')
    parser.add_argument('--model', default='llama7B_adapter', type=str, metavar='MODEL', help='Name of model to train')
    parser.add_argument('--adapter_layer', type=int, default=24, metavar='LENGTH', help='the number of adapter layer')
    ...
    parser.add_argument('--weight_decay', type=float, default=0.16, help='weight decay (default: 0.05)')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss = train_one_epoch(model, data_loader_train, optimizer, epoch, loss_scaler, args=args)
        print("loss:", loss)
        # val_stats = val_one_epoch(model_without_ddp, data_loader_val, optimizer, epoch, args=args)
        acc = val_one_epoch(model_without_ddp, data_loader_val, optimizer, epoch, args=args)
        print("acc:", acc)
        # if args.output_dir and best_acc < val_stats['acc']:
        #     best_acc = val_stats['acc']
        if args.output_dir and best_acc < acc:
            best_acc = acc
